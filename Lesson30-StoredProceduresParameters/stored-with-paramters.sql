...
--   that have average > a given value in a given semester. [*can check parameters in one
--   conditional expression and a common message printed if any of them are missing*]


--3. Create a stored procedure called “NotInACourse” that lists the full names of the staff that
--   are not taught a given CourseID.
...
--5. Create a stored procedure called “ListaProvince” to list all the student names that are
--   in a given province.
...
--7. Create a stored procedure called “PaymentTypeCount” to select the count of payments made for
--   a given payment type description.


--8. Create stored procedure called “Class List” to select student Full names that are in a course
--   for a given SemesterCode and CourseName.
