...
--3. Create a stored procedure called ‘Updateclub’ to update a club record.
--   Do not update the primary key!


--4. Create a stored procedure called ‘ClubMaintenance’. It will accept parameters
--   for both ClubID and ClubName as well as a parameter to indicate if it is an insert,
--   update or delete. This parameter will be ‘I’, ‘U’ or ‘D’ (insert, update, or delete
--   a record accordingly).
--   Focus on making your code as efficient and maintainable as possible.


--5. Create a stored procedure called ‘RegisterStudent’ that accepts StudentID and
--   OfferingCode as parameters. If the number of students in that Offering are not greater
--   than the Max Students for that course, add a record to the Registration table and add
--   the cost of the course to the student’s balance. If the registration would cause the
--   Offering to have greater than the MaxStudents raise an error.


--6. Create a procedure called ‘StudentPayment’ that accepts Student ID, paymentamount,
--   and paymentTypeID as parameters. Add the payment to the payment table and adjust the
...
--   Fire the staff member by updating the record for that staff and entering today’s date as
--   the DateReleased.
